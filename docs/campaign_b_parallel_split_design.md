...
> | **96** | 統合 backlog-aware end-to-end（**任意・スタンドアロン**；97 と排他） |
...
3. `96_campaign_b_end_to_end.ipynb`（**任意・スタンドアロン**；89∥97 の代替）
   - 単独利用 OK。backlog throttle は効率化のオプションであり必須ではない。
   - M3 keep-latest / strip reclaim は 97 と同じ既定 ON
     （`docs/campaign_b_m3_storage_reclaim.md`）。
...
2. 97 のデフォルトは既存 M2-ready / READY_FOR_M3 backlog の drain
   （`drain_existing_backlog=True` / `skip_screening=True`）。
3. **任意・スタンドアロン:** Notebook **96 だけ**（統合スケジューラ；89∥97 の代替）。
   throttle は必須ではない。**97 と同時起動しない。**
