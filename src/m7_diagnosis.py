...
def diagnose_m6_package(
    ...
    *,
    ...
    if isinstance(labels, list) and isinstance(entries, list):
        for row in entries:
            ...
            for cell in row:
                ...
                if hi is not None:
                    total += hi
            row_sum_uppers.append(str(total))
    ...
    return {
        ...
        'q_cert_upper': str(q_hi) if q_hi is not None else None,
        'parent_m5_q_cert_upper': str(parent_m5_q) if parent_m5_q is not None else None,
        'inherited_majorant': inherited,
        'composition_policy': policy,
        'open_ledger_leaves': open_leaves,
        'max_row_sum_upper': str(max_row),
        'row_sum_uppers': row_sum_uppers,
        'recommended_campaign': 'A' if DIAG_D0 in codes else 'A',
        'notes': (
            'q_cert >= 1 is a certificate failure of the declared majorant, '
            'not a proof that the true RG map is expansive.'
        ),
        'next_actions': [
            'Replace PARENT_ONE_STEP_INHERITANCE with DIRECT_MULTI_STEP_PRODUCT '
            'or STAGE_DEPENDENT_WEIGHTED_PRODUCT.',
            'Run S0 Perron / Collatz vector search on the final majorant.',
            'If estimated spectral radius stays >= 1, promote Campaign B (rank/cutoff).',
        ],
    }
