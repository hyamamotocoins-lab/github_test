...
GPU matrix-free Triad-ATRG pilot、M4 の forward derivative と error ledger まで実行済みです。
人間向けの判定と参照確認は次を開いてください。

- `notebooks/35_m2_execution_reference_report.ipynb`
- `notebooks/45_m3_execution_reference_report.ipynb`
- `notebooks/51_m4_resume_existing.ipynb`（既存M4 runの人手再開）
- `notebooks/55_m4_execution_reference_report.ipynb`
...
を維持します。M1/M2 の厳密結果、M3 の再現可能な探索結果、M4 の forward derivative 回帰も、
4次元 RG や mass gap の certification ではありません。
...
| M7 | 実装・Paperspace 実行可 | Campaign A/B scheme search：`q_cert_upper<1` の独立検証済み scheme を探索 |
...
へ 8 個の M4→M5 handoff obligations の評価が書かれます。以前 4 項を意図的に OPEN にしていた理由は、
未証明の 0 埋めを禁止する fail-closed 方針だからです。現在は次の **限定スコープ** で 8 項とも
`RIGOROUS` に閉じ、live `one_step_certificate/` と `reports/M5_acceptance.json` を書いて
M6 ゲートを開けます。

| obligation | 閉鎖の意味（スコープ） |
|---|---|
| GPU rounding / backward error | 凍結 16×16 pipeline の CPU 再計算残差 |
| M3 RSVD projection residual | 凍結 float64 演算子 vs RSVD 因子の Fraction Frobenius |
| basis variation residual | 固定基底の alias（projection residual と同上界） |
| normalization / denominator error | 凍結 center Frobenius スケールが正 |
| cutoff and rank dependence | 凍結 (cutoff, rank) 上の証明書。族全体の残差ではない |
| initial representation tail | M1 2D analytic tail × 接触数 telescoping lift（`j2_max=1`） |
| input radius propagation | singleton 入力球 `r_0=0` → 伝播 0 |
| omitted fusion / channel tail | 凍結 `j2_max=1` の projector 網羅（打ち切り外は initial tail 側） |

成功時の成果物:

```text
reports/M5_obligation_report.json   # all_closed=true
artifacts/one_step_certificate/    # Collatz + independent verifier
reports/M5_acceptance.json          # phase=M5_COMPLETE → M6 開始可
...
```text
VALIDATED_RG_M7_RUN_ID=M7-20260720T074400Z-9f2a18c6d401   # Campaign A
VALIDATED_RG_M7B_RUN_ID=M7-20260720T080000Z-b7c4e91a2b85  # Campaign B (S2)
```
...
（`lineage_mode=plan_only` 既定。`fixture_residual` は CPU コントローラ試験専用）。
M4 regroup は平方数 `projected_rank` を要求するため、非平方 rank は次の平方へ引き上げる。

現行 paperspace 設計は親 M5 one-step majorant を最終値として継承するため、親と同じ
`q_cert` になり得ます。これは multi-step 積 \(B_{K-1}\cdots B_0\) の鋭化や段ごと重み再最適化を
まだ使っていないことを意味し、真の写像の非収縮確認ではありません。Campaign A 枯渇後は
Campaign B の lineage plan を実行し、子 M6 を独立検証します。
...
run ID: M4-20260720T021737Z-b9c9514fed11
phase: M4_COMPLETE
milestone status: BLOCKED_MATH
enclosure status: BLOCKED_MATH
checkpoint: /storage/validated_4d_su2_rg/runs/M4-20260720T021737Z-b9c9514fed11/checkpoints/ckpt_000014
...
acceptance: runs/.../reports/M5_acceptance.json
status: re-run 60 after pull — closes all 8 obligations, writes acceptance for M6
```
...
M4 の実測要約：5個の symmetry-reduced source channel、固定基底 FP64 forward AD、product rule、
完全な normalization derivative、12項の error DAG を実装しました。全5 channel の centered
finite-difference regression は収束し、最大 final relative error は `1.4711e-7` です。ただし
finite difference は proof bound ではありません。決定論的境界が8項目不足し、rigorous leaf は
M2 exact basis equivalence の1項だけなので、one-step enclosure は fail-closed で停止しています。
GPU peak は約32.0 MiB、checkpoint は65,970 bytes（save 0.215 s）でした。

今回の M4 初回セッションだけは、ユーザー指定どおり1時間30分以降に長い仕事を始めず、1時間45分で
drain、1時間50分で final save、2時間で hard return する設定です。次回以降の再開は通常設定に戻り、
5時間で長い仕事を止め、5時間15分で drain、5時間20分で final save、5時間30分で hard return します。
どちらも15分周期と work-item/phase 境界で永続 checkpoint を保存します。

既存M4 runを人間が再開・再確認するときは、ハッシュ固定済みの `50_m4_derivatives.ipynb` を
編集せず、fresh kernelで `notebooks/51_m4_resume_existing.ipynb` を上から実行してください。
承認済み永続markerを検査してkernel内の環境変数を復元し、通常の5時間30分ポリシーで
`M4-20260720T021737Z-b9c9514fed11` を再開します。
...
│   ├── 35_m2_execution_reference_report.ipynb
│   ├── 40_m3_gpu_triad_atrg.ipynb
│   ├── 45_m3_execution_reference_report.ipynb
│   ├── 50_m4_derivatives.ipynb
│   ├── 51_m4_resume_existing.ipynb
│   ├── 55_m4_execution_reference_report.ipynb
│   ├── 60_m5_one_step_certificate.ipynb
│   └── 70_m6_multistep_certificate.ipynb
├── audit/
│   ├── m0_accepted_parent.json
│   ├── m1_accepted_parent.json
│   ├── m2_accepted_parent.json
│   └── m3_accepted_parent.json
├── requirements/
│   └── paperspace.txt
├── validated_4d_su2_rg_codex_design.md
├── AGENTS.md
...
比較を実装済みです。M4 では5 source channel の forward tangent、完全な product/normalization
derivative、誤差項の provenance DAG を実装済みですが、浮動小数点回帰は厳密 bound ではありません。
次は未実装または数学的に未解決です。

- 4次元 RG step の a posteriori enclosure。
- low-rank residual、GPU rounding error、normalization lower bound の厳密評価。
- source tangent の deterministic enclosure、influence matrix、Collatz–Wielandt bound。
- initial representation tail、input radius、omitted fusion/channel tail の M4 operator norm への移送。
- cutoff/rank 依存性の検証。
- P0 の最終的な transitive checkpoint hash-chain certificate package。
- thermodynamic/continuum bridge B1–B7。

したがって M1–M4 report は `CERTIFIED`、`TAIL_BOUNDED` を4次元全体の verdict として出しません。
...
milestone、phase、`status=PASS` を照合します（M0–M4 は `certification_status=NOT_CERTIFIED`、
M5→M6 は `NOT_CERTIFIED` または `ONE_STEP_CERTIFIED` を受理）。直前段階の
...
M1、M2、M3 は実環境 report を確認して受理済みです。M3 の `CORE_REPRODUCED` は certification
ではなく、その制限を保持したまま M4 親として固定しました。M4 は全18実装ゲートに PASS しましたが、
`notebooks/55_m4_execution_reference_report.ipynb` が示す8個の missing deterministic bound により
`BLOCKED_MATH` です。`M4_acceptance.json` の `status=PASS` は実装ゲートの意味であって、M5開始許可や
数学的 enclosure の意味ではありません。M5 は開始せず、欠落境界を解消して M4 report を再レビュー
した後にだけ進行を判断します。既存M4 runの再開には `51_m4_resume_existing.ipynb` を使用します。
