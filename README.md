...
最初の setup notebook と各段階 notebook は、最初のコードセルで `pytest` の有無を確認し、
ない場合だけ `pytest>=8` を導入します。PyTorch/CUDA と NumPy は既存環境を変更しません。

M1 の対象は、二次元 pure SU(2) gauge theory の exact benchmark と、4次元計算に先立つ
representation tail 基盤です。4次元 armillary tensor、Triad-ATRG、influence matrix、
mass-gap certificate はまだ実装しません。
...
を維持します。M1 の受理は有限個の係数・tail・二次元 recurrence の検証を意味するだけで、
...
| M1 | 実行完了・人手受理待ち | 全受理ゲート PASS、`ckpt_000014`、`NOT_CERTIFIED` |
| M2 | 受理ゲートのみ・未実装 | M1 受理後の low-cutoff 4D dense/armillary equivalence |
| M3 | 受理ゲートのみ・未実装 | M2 受理後の GPU matrix-free Triad-ATRG pilot |
...
M1 の全 acceptance test と report を人間が確認するまで M2 を開始しません。
...
- 4次元 armillary tensor と dense/armillary equivalence。
- 4次元 RG step enclosure。
- low-rank residual、rounding error、normalization lower bound。
...
したがって M1 report は `CERTIFIED`、`TAIL_BOUNDED` を4次元全体の verdict として出しません。
...
現在 M1 の実行と自動 acceptance gate は完了していますが、人間による M1 report の受理前なので、
M2–M6 は受理ゲートと実装境界だけです。これは欠落をゼロや PASS で埋めないための意図的な停止です。
各直前段階の実環境 report を人間が受理した後、
対応する notebook だけに実装、test、checkpoint/restart、独立 verifier、acceptance report を
追加します。ゲートを通過しただけでは現在段階の完了でも certification でもありません。
