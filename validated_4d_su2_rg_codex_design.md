...
**設計版:** 0.1.0  
**主成果物:** `notebooks/00_project_setup.ipynb` と `notebooks/10_m0_accepted_audit.ipynb`–`70_m6_multistep_certificate.ipynb`  
...
段階別 Notebook を主実行画面とする。旧 `validated_4d_su2_rg_gpu_driver.ipynb` は案内専用とし、
計算本体を置かない。実装済み M1 Notebook 内の `%%writefile` cellで module を project root に生成する。

```text
project_root/
├── validated_4d_su2_rg_gpu_driver.ipynb  # 互換案内
├── notebooks/
...
`src/` と `tests/` は Notebook 内の module 生成 cell を正本とする。外部 module を手で変更した
場合は、次回 Notebook 実行で上書きされる可能性を明記する。cache、run、checkpoint、report は
project root ではなく `VALIDATED_RG_PERSIST_ROOT`（Paperspace では `/storage`）に置く。
